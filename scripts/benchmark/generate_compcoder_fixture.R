...
if (length(args) != 3L) {
  stop(
    "Usage: generate_compcoder_fixture.R R_LIBRARY REPLICATE OUTPUT_DIRECTORY",
    call. = FALSE
  )
}

r_library <- normalizePath(args[[1L]], mustWork = TRUE)
replicate_id <- as.integer(args[[2L]])
output_directory <- normalizePath(args[[3L]], mustWork = FALSE)
.libPaths(c(r_library, .libPaths()))

if (is.na(replicate_id) || replicate_id < 1L || replicate_id > 10L) {
  stop("Replicate must be an integer from 1 through 10.", call. = FALSE)
}
suppressPackageStartupMessages(library(compcodeR))
dir.create(output_directory, recursive = TRUE, showWarnings = FALSE)

seed <- 5000L + replicate_id
set.seed(seed)
simulation <- suppressMessages(suppressWarnings(generateSyntheticData(
  dataset = "p0_nb_balanced",
  n.vars = 5000,
  samples.per.cond = 6,
  n.diffexp = 500,
...
fixture <- list(
  fixture_id = sprintf("compcodeR-1.48.0-p0-nb-balanced-replicate-%02d", replicate_id),
  ...
  outlier_probabilities = list(random_high = 0, random_low = 0, single_high = 0, single_low = 0),
  design = "~ condition",
